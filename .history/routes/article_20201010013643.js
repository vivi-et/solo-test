...
router.get('/', async (req, res) => {
  ...
  Object.keys(articles).forEach((key) => {
    articles[key].moments = moment(articles[key].createdAt).format('MMM Do YY');
  ...
  upload(req, res, async (err) => {
    ...
    if (err) {
      ...
      await Article.create({
        title,
        text,
        image: './public/uploads/imgs/article/noimage.jpg',
      });
      ...
      await Article.create({
        title,
        text,
        image: `./public/uploads/imgs/article/${req.file.image}`,
      });
    }
