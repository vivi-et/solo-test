...
router.get('/?:page', async (req, res) => {
  ...
  const navBarIndexOffset = Math.ceil(page / navBarIndexLimit); // offset previous indexes like < 5 6 7 8 >
  ...
  if (page > totalPage) {
    // if at last page, backtrack to previous indexes
  ...
  Object.keys(articles.rows).forEach((key) => { // format createdAt to more readable time
    articles.rows[key].moments = moment(articles.rows[key].createdAt)
      .startOf('hour')
      .fromNow();
  });
