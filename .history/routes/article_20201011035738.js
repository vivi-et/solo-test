...
router.get('/?:page', async (req, res) => {
  ...
  const limit = size || 6; // if size isn't specified, set size to 5
  const offset = page ? ((page - 1) * limit) : 0; // skip previous items
  const navBarIndexLimit = 3; // how many page indexes to show in navigation bar
  const navBarIndexOffset = Math.floor(page / navBarIndexLimit); // skip previous indexes
  ...
  const totalPage = Math.floor(articles.count / limit);

  const navBarIndexArray = [];
  // set index bar for view
  for (let i = 0; i < navBarIndexLimit; i += 1) {
    const nextPage = (i + navBarIndexOffset * navBarIndexLimit) + 1;

    console.log(nextPage);
    navBarIndexArray.push(nextPage);
    if (nextPage >= totalPage) {
      console.log(`borken at ${nextPage}`);
      break;
    }
  }
